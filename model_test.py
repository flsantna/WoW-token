    ...
    if test_type == 0:
        ...
        for key in range(len(list_key)):
            ...
            plot_graph(true_data=np.append(data_train_series_y, data_in_series_y, axis=0),
                       ...
                       title=list_key[key],
                       file_name="True data as look_back "+list_key[key],
                       plot_predict=True, plot=True)
...
# and calculating it's RMSE (Reduced Mean Squared Error).
        ...
        for key in range(len(list_key)):
            ...
            plot_graph_array(true_data=all_true_data[len(predict_days) * key + len(predict_days) - 1:
                                                     len(predict_days) * key + len(predict_days)][0],
                             predicted_data=all_predicts[len(predict_days) * key:
                                                         key * len(predict_days) + len(predict_days)],
                             dates=dates_to_plot_all,
                             title=list_key[key],
                             plot=False)
